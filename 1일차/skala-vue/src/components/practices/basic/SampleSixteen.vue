...
const runTask1 = () => {
  ...
  // [미션]
  // 1. members 배열에 '박부산'이 포함(includes)되어 있는지 확인하여 담기 변수(memberContainsPark)에 담기
  ...
  // 3. 추출한 변수들을 템플릿 리터럴을 사용하여 result1.value에 주입하기.
...
const runTask2 = () => {
  ...
  // 2. 옵셔널 체이닝(?.)과 널 병합 연산자(??)를 연쇄로 사용하여, newProduct.preview 값이 null이거나 undefined일 때 '이미지 준비중'이라는 문자열을 변수 'imgStatus'에 담기.
  const imgStatus = newProduct?.preview ?? '이미지 준비중'

  // 3. 널 병합 연산자(??)를 활용하여, newProduct.stock이 0일 때 안전하게 변수 'finalStock'에 담기도록 조치하기.
  // (참고: || 연산자를 쓰면 0을 false로 인식해버리지만, ?? 연산자는 0을 유효한 값으로 지켜줍니다.)
  const finalStock = newProduct.stock ?? '재고 정보 없음'
...
<template>
  <div class="quiz-container">
    <h2>챌린지 16. ES6+ 문법 종합 실습 (구조분해할당, 데이터 방어, Async/Await)</h2>
