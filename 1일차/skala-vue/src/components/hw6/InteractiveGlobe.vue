...
const getGlobeData = () => {
  ...
  if (!list.some(item => item.id === 'Dokdo' || item.label.includes('독도'))) {
    list.push({ lat: 37.2425, lng: 131.8669, size: 1.5, color: '#f1c40f', label: '🇰🇷 독도 (우리땅)', status: '정보 불러오는 중', id: 'Dokdo' })
  }
  // 🏝️ 제주도 Fallback 핀
  if (!list.some(item => item.id === 'Jeju' || item.label.includes('제주'))) {
    list.push({ lat: 33.4996, lng: 126.5312, size: 1.3, color: '#2ecc71', label: '🏝️ 제주도', status: '정보 불러오는 중', id: 'Jeju' })
  }
  // ⛰️ 울릉도 Fallback 핀
  if (!list.some(item => item.id === 'Ulleungdo' || item.label.includes('울릉'))) {
    list.push({ lat: 37.4847, lng: 130.9010, size: 1.3, color: '#3498db', label: '⛰️ 울릉도', status: '정보 불러오는 중', id: 'Ulleungdo' })
  }
      ...
      .then(countries => {
        ...
        if (sk) {
          ...
          sk.geometry.coordinates.push(
            ...
            createCirclePolygon(37.48, 130.90, 0.08, 0.1)[0],
        ...
        const countryLabels = countries.features
          ...
          .map(f => {
            ...
            return { lat, lng, text: f.properties.NAME_KO || f.properties.NAME, size: 0.5, color: 'rgba(255,255,255,0.75)' }
          })
          .filter(l => l.lat !== 0 && l.lng !== 0)

        globeInstance.labelsData(countryLabels).labelLat(d => d.lat).labelLng(d => d.lng)
          .labelText(d => d.text).labelSize(0.5).labelColor(d => d.color)
  ...
  <div class="globe-container-wrapper">
    <div class="globe-wrapper" ref="globeContainer">
      ...
      <div v-if="globeFailed" class="globe-fallback">
        ...
        <p class="fallback-desc">Chrome 설정 → 시스템 → "하드웨어 가속 사용" 활성화 후 재시작하면 3D 지구본이 표시됩니다.</p>
...
.fallback-title { font-size: 18px; font-weight: 700; margin: 0 0 8px; }
.fallback-desc { font-size: 13px; opacity: 0.8; line-height: 1.6; margin: 0 0 16px; max-width: 380px; }
.fallback-link {
...
.fallback-link:hover { background: rgba(255,255,255,0.25); }
.globe-legend {
  position: absolute;
  bottom: 20px;
  right: 20px;
...
.legend-header:hover { background: rgba(0, 0, 0, 0.1); }
.legend-content { padding: 10px 15px 10px 15px; }
.legend-toggle-icon { margin-left: 10px; font-size: 10px; color: #909399; }
.legend-item {
  display: flex;
  align-items: center;
  margin-bottom: 6px;
